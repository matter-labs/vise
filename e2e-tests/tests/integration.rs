...
async fn start_app(prom_format: bool) -> anyhow::Result<(Child, u16)> {
...
async fn test_app(prom_format: bool) -> anyhow::Result<()> {
    init_logging();

    let (_app_process, app_port) = start_app(prom_format).await?;
